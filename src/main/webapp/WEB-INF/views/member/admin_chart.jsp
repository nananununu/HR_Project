...
//         empCardSort();
      ...
      const createEmployeeListItem = (item) => {
         ...
         html += '<li class="employee-li" data-employee-key=' + item.employee_id + ' data-line-name=' + item.line_name + ' data-department-name=' + item.department_name + '>';
         ...
         const employeeProfile = document.querySelector('.employee-info-profile-img');
           ...
           employeeProfile.setAttribute('src','${root}upload/' + employeeInfo.employee_profile_picture)
            ...
            if (employeeInfo.employee_status === '휴직중') {
               ...
               document.querySelector('.employee-inner-status-icon').classList.remove('status-active');
            } else {
               document.querySelector('.employee-inner-status-icon').classList.add('status-active');
               document.querySelector('.employee-inner-status-icon').classList.remove('status-inactive');
            }
         }
      }
